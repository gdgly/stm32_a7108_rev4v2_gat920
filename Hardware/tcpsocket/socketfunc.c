...
u16 SOCKET_ObtainPacketData(u8 *addr)
{
	...
	for (i = 0; i < OUTPUT_MAX; i++) {																	//遍历找寻配置过的车道号
		if (SocketDataPacket[i].OutputID != 0) {
			...
			if ((SOCKET_Convert2Byte((u8 *)&addr[datalength], SocketDataPacket[i].AvgOccupancy)) == SOCKET_FALSE) {			//填入平均占有时间
				return SOCKET_FALSE;
			}
			else {
				datalength += 2;																	//地址偏移2byte
			...
			if ((SOCKET_Convert2Byte((u8 *)&addr[datalength], SocketDataPacket[i].AvgHeadTime)) == SOCKET_FALSE) {			//填入平均车头时距
				return SOCKET_FALSE;
			}
			else {
				datalength += 2;																	//地址偏移2byte
